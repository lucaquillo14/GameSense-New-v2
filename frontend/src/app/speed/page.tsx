...
export default function SpeedAnalysisPage() {
  ...
  return (
    <AppShell>
      <section className="hero-glow mx-auto max-w-3xl px-5 py-14 fade-in">
        <Link
          href="/"
          className="mb-8 inline-flex items-center gap-1.5 text-sm text-[#6b7a99] transition-colors hover:text-[#eef2ff]"
        >
          <ArrowLeft size={15} /> All analysis modes
        </Link>
        ...
        <div className="mb-9">
          ...
          <h1 className="display text-4xl text-[#eef2ff] sm:text-5xl">
            Measure top speed &amp; <span className="gradient-text">distance</span>
          </h1>
          <p className="mt-4 max-w-xl text-lg leading-relaxed text-[#8b95a7]">
            Upload match video, pick your player, and get sprint speed, distance covered, and sprint
            counts — automatically tracked frame by frame.
          </p>
        </div>
        ...
        <div className="card p-6">
          <label className="dropzone-hover group relative flex min-h-72 w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-cyan-500/20 bg-cyan-500/[0.02] px-6 py-10 text-center transition-all duration-300 hover:bg-cyan-500/[0.04]">
            ...
            <span className="font-display text-xl font-semibold text-[#eef2ff]">
              {checking ? "Checking video…" : file ? file.name : "Drag & drop your match clip"}
            </span>
          ...
          {localMeta && (
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <MetaPill icon={<Maximize2 size={13} />} label={`${localMeta.width}×${localMeta.height}`} />
            </div>
          )}
        ...
        <p className="mt-5 text-center text-xs text-[#3a4560]">
          FPS is detected after upload. Clips longer than {MAX_VIDEO_DURATION_S} seconds are rejected
          before upload.
        </p>
      </section>
    </AppShell>
